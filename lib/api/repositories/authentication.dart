...
abstract class StravaAuthenticationRepository {
  Future<void> authenticate({
    ...
    if (token == null) {
      ...
      print("marnv");
      List<StravaAuthScope> oldScopes =
          StravaAuthScopeHelper.generateScopes(token.scopes ?? "");
      ...
      if (isScopesAreSame) {
        if (SessionManager.getInstance.isTokenExpired(token)) {
          return _refreshAccessToken(SessionManager.getInstance.clientId,
                  SessionManager.getInstance.secret, token.refreshToken)
  ...
  Future<void> _completeAuthentication(
    ...
    return _getStravaCode(
            ...
            forceShowingApproval: forceShowingApproval)
        .then((code) {
      return _requestNewAccessToken(SessionManager.getInstance.clientId,
              SessionManager.getInstance.secret, code)
          .then((token) {
        return SessionManager.getInstance
            .setToken(token: token, scopes: scopes);
      });
    });
  }
  ...
  Future<Token> _requestNewAccessToken(
  ...
  Future<RefreshToken> _refreshAccessToken(
